...
- [x] ランダムプレイヤーに80%以上勝利できるab探索を今週中に -> できた(97 percent)
- [ ] 

NegaMax depth=1が今のところ一番強いし爆速という知見があります。
なんか流石にMinMax深くすると弱くなるは信じがたくて、2段目以降がバグってる結果こうなってる気がするんだけど、
速くてそこそこ強いAIはbaselineとして有り難いので別クラスに抽出しようと思います。

Hashes()の実装もいいけど、評価関数の分離とともに明らかにこれがどうバグってるかをちゃんと調査するのが先だよな…


## ToDo

* 評価関数をEvaluator::Evaluateに分離してテンプレートで取るように
* NegaMax depth=1を別クラスに分離
