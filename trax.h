...
class Position {
 public:
  ...
  std::string ToString64x64() {
    std::vector<std::string> result_field(64, std::string(64, '0'));
    const int offset_x = max_x_ / 2 - 32;
    const int offset_y = max_y_ / 2 - 32;
    for (int i_x = 0; i_x < 64; ++i_x) {
      for (int j_y = 0; j_y < 64; ++j_y) {
