...
class NegaMaxSearcher : public Searcher {
 public:
  ...
  virtual Move SearchBestMove(const Position& position) {
    ...
    std::sort(moves.begin(), moves.end(), [](std::pair<int, Move>& lhs, std::pair<int, Move>& rhs) {
        return lhs.first < rhs.first;
        });

    for (auto&& score_move : moves) {
      std::cerr << score_move.first << " " << score_move.second.notation() << std::endl;
    }
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (StartSelfGame(&negamax_searcher, &random_searcher, /* verbose = */ true)) {
      ++red;
    } else {
      ++white;
    }
